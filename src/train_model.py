...
df = create_target(df, steps=3)   # predice 30 min adelante
...
print(f"\n📈 MAE promedio CV:  {np.mean(mae_folds):.3f} ± {np.std(mae_folds):.3f} °C")
print(f"📈 RMSE promedio CV: {np.mean(rmse_folds):.3f} ± {np.std(rmse_folds):.3f} °C")
